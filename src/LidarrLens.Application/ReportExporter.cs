...
public sealed class ReportExporter : IReportExporter
{
    ...
    {
        var sb = new StringBuilder("id,artist,source,type,status,confidence,title,source_url,artwork_url,action\n");
        foreach (var f in findings) sb.AppendLine(string.Join(',', new[] { f.Id, f.ArtistName, f.SourceName, f.Type.ToString(), f.Status.ToString(), f.Confidence.ToString(), f.CopyReady?.Title, f.SourceUrl, f.CopyReady?.ArtworkUrl, f.SuggestedAction }.Select(Csv)));
    ...
    {
        var cards = string.Join("\n", findings.Select(f => $"<article class='card' data-search='{WebUtility.HtmlEncode((f.ArtistName + " " + f.CopyReady?.Title + " " + f.SourceName).ToLowerInvariant())}'><div class='row'><h2>{E(f.CopyReady?.Title ?? f.Type.ToString())}</h2><span class='badge'>{E(f.Status.ToString())}</span></div><p><strong>{E(f.ArtistName)}</strong> · {E(f.Type.ToString())} · {E(f.Confidence.ToString())}</p><p>{E(f.SuggestedAction)}</p><details><summary>Evidence and copy-ready data</summary><pre>{E(BuildVisibleData(f))}</pre><button onclick=\"copyText(this)\">Copy visible data</button>{(f.SourceUrl is null ? "" : $" <a href='{E(f.SourceUrl)}'>Source</a>")}{(f.CopyReady?.ArtworkUrl is null ? "" : $" <a href='{E(f.CopyReady.ArtworkUrl)}' target='_blank' rel='noopener noreferrer'>Artwork</a>")}</details></article>"));
    ...
    private static string BuildVisibleData(AuditFinding finding) => finding.CopyReady is null
        ? string.Join("\n", finding.Evidence)
        : $"Release title: {finding.CopyReady.Title}\nArtwork URL: {finding.CopyReady.ArtworkUrl ?? "(none)"}\n\n{string.Join("\n", finding.CopyReady.Tracklist)}\n\nEdit note: {finding.CopyReady.EditNote}";
}
